    ...
    public class UploadPartnerMaster
    {
            ...
            try
            {
                ...
                if (dataSetFileData != null && dataSetFileData.Tables[0].Rows.Count > 0)
                {
                    foreach (DataRow dataRowFileData in dataSetFileData.Tables[0].Rows)
                    {
                        ...
                        if (_uploadMasterCommonFactory().DoDownloadBLOB(Convert.ToString(dataRowFileData["FileName"]), Convert.ToString(dataRowFileData["ContainerName"]), "Partners", out FilePath))
                        {
        ...
        public async Task<bool> ProcessFile(int srNotblMasterUploadSummaryLog, string filepath)
        {
            ...
            try
            {
                ...
                _uploadMasterCommonFactory().RemoveEmptyRows(dataTablePartnerExcelData);
        ...
        public async Task<DataSet?> ValidateAndInsertPartnerMaster(int srNotblMasterUploadSummaryLog, DataTable shipperConsigneeType, string errorInSp)
        {
            ...
            {
                // clsLog.WriteLogAzure("Message: " + exception.Message + " Stack Trace: " + exception.StackTrace);
                _logger.LogError($"Message: {exception.Message} Stack Trace: {exception.StackTrace}", exception.Message, exception.StackTrace);
                return dataSetResult;
            }
        }
    }
