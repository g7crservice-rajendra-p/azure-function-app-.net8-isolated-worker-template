...
namespace QidWorkerRole
{
    class cls_SCMBL
    {
        ...
        private bool InsertFFRToOutBox(string Body, string FromId, string ToID)
        {
        ...
        private DataSet GetAWBDetailsForFSA(string AWBNo, string msg, string AWBPrefix)
        {
        ...
        private DataSet getFlightDetailsForFSA(string AWBNo, string FlightNo, string AWBPrefix)
        {
        ...
        private bool addMsgToOutBox(string subject, string Msg, string FromEmailID, string ToEmailID, string agent, int refNo)
        {
            bool flag = false;
            try
            {
        ...
        private bool InsertFFAIntoOutbox(object[] RateCardInfo)
        {
            bool flag = false;
            try
            {
        ...
        private bool validateAndInsertUCMData(MessageData.UCMInfo ucmdata, MessageData.ULDinfo[] uld)
        {
            bool flag = false;
            try
            {
        ...
        private bool validateAndInsertSCMData(MessageData.SCMInfo[] scm)
        {
            bool flag = false;
            try
            {
        ...
        public DataSet getAcceptedBookingData(string pName)
        {
            DataSet ds = new DataSet();
            try
            {
        ...
        public bool addBookingFromMsg(string strMsg, int refNO, string strMessageFrom, out string msgType, string strFromID, string strStatus, string PIMAAddress, out string Errmsg)
        {
            ...
            DateTime flightdate = System.DateTime.UtcNow;
                ...
                if (strMsg != null)
                {
                    if (strMsg.Trim() != "")
                    {
                        ...
                        if (arrTempMessage.Length > 1)
                        {
                            ...
                            else if (genericFunction.GetConfigurationValues("GMAILMailINServer").ToUpper() == "OUTLOOK.OFFICE365.COM")
                            {
                        ...
                        else if (strMsg.Trim().StartsWith("FWB", StringComparison.OrdinalIgnoreCase))
                        {
                            ...
                            if (flag == true && cimpMessageValidation.ValidateFWB(strMsg, refNO, out ErrorMsg))
                        ...
                        else if (strMsg.Trim().StartsWith("UCM", StringComparison.OrdinalIgnoreCase))
                        {
                            ...
                            if (flag == true)
                                flag = validateAndInsertUCMData(ucmdata, uld);
                            #endregion
                        }
                            ...
                            || strMsg.Trim().StartsWith("PSN", StringComparison.OrdinalIgnoreCase))
                        {
                            try
                            {
                                ...
                                objDecode.DecodeCustomsMessage(strMsg, strOriginalMessage, strMessageFrom, out msgType);
                            }
                            catch (Exception ex)
                            { clsLog.WriteLogAzure(ex); }
                        }
                        ...
                        else if (strMsg.Trim().StartsWith("UNB") && (strMsg.Trim().Contains("STS++74")) || (strMsg.Trim().Contains("STS++24")) || (strMsg.Trim().Contains("STS++21")) || (strMsg.Trim().Contains("STS++40")))
                        {
                            CarditResiditManagement crdmanage = new CarditResiditManagement();
                            string CarditMessage = strMsg.Replace("$", "");
                            flag = crdmanage.EncodeAndSaveResditMessage(CarditMessage);
                        }
                        ...
                        else if (strMsg.Trim().StartsWith("SCM", StringComparison.OrdinalIgnoreCase))
                        {
                            ...
                            {
                                flag = false;
                                flag = validateAndInsertSCMData(scm);
                            }
                            #endregion
                        }
                            ...
                            if (strMsg.Trim().StartsWith("ASM", StringComparison.OrdinalIgnoreCase))
                            {
                                ...
                                for (int i = 0; i < asmBodyLine.Length; i++)
                                {
                                    if (asmBodyLine[i].ToString().Trim() != string.Empty)
                                    {
                                        ...
                                        strMsg = strMsg.Replace("$$", "$");
                                        strMsg = strMsg.Trim('$');
                                        objAsm.ToASM(strMsg, refNO, strOriginalMessage, strMessageFrom, out flag);
                                    }
                                }
                            }
        ...
        private void ReconstructFFRoute(MessageData.consignmnetinfo[] objConsInfo, ref MessageData.FltRoute[] objRouteInfo, int srno)
        {
            try
            {
                ...
                MessageData.FltRoute[] objRouteInfo_Temp = new MessageData.FltRoute[0];
                MessageData.FltRoute flight_Temp = new MessageData.FltRoute("");
        ...
        public void RelayMessages(string MessageName, String strMsg, string carrierCode, DateTime flightdate, bool FltLevel, string org = "", string dest = "", string fltnumber = "", string awbnumber = "")
        {
            ...
            DataSet dsconfiguration = genericFunction.GetSitaAddressandMessageVersion(carrierCode, MessageName, "AIR", org, dest, fltnumber, string.Empty);
            ...
            try
            {
                if (strMsg.Length > 3)
                {
                    ...
                    if (SitaMessageHeader.Trim().Length > 0)
                        genericFunction.SaveMessageOutBox("SITA" + MessageName, SitaMessageHeader.ToString() + "\r\n" + strMsg, "", "SITAFTP", org, dest, fltnumber, flightdate.ToString());

                    if (SFTPHeaderSITAddress.Trim().Length > 0)
                        genericFunction.SaveMessageOutBox("SFTP" + MessageName, SFTPHeaderSITAddress.ToString() + "\r\n" + strMsg, "", "SFTP", org, dest, fltnumber, flightdate.ToString());

                    if (strEmailid.Trim().Length > 0)
                        genericFunction.SaveMessageOutBox(MessageName, strMsg, "", strEmailid, org, dest, fltnumber, flightdate.ToString());
        ...
        public bool EncodeFBLForSend(string POL, string FlightNo, string FlightDate, string FromEmailID, string ToEmailID)
        {
            ...
            try
            {
                ...
                dsData = getFBLData(POL, FlightNo, FlightDate);
        ...
        public bool EncodeSendingFBL(string POL, string FlightNo, string FlightDate, string FromEmailID, string ToEmailID)
        {
            MessageData.fblinfo objFBLInfo = new MessageData.fblinfo("");
            MessageData.unloadingport[] objUnloadingPort = new MessageData.unloadingport[0];
            MessageData.consignmnetinfo[] objConsInfo = new MessageData.consignmnetinfo[0];
            ...
            try
            {
                ...
                dsData = getFBLData(POL, FlightNo, DateTime.Parse(FlightDate).ToString("MM/dd/yyyy"));
        ...
        public string EncodeFFM(string DepartureAirport, string FlightNo, DateTime FlightDate, string MsgVer)
        {
            ...
            try
            {
                ...
                DataSet dsData = new DataSet();
                DataSet ds = new DataSet();
                ds = GetFlightInformationforFFM(DepartureAirport, FlightNo, FlightDate);
                if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    {
                        MessageData.unloadingport objTempUnloadingPort = new MessageData.unloadingport("");
                        dsData = GetRecordforGenerateFFM(DepartureAirport, FlightNo, FlightDate, drdestination["DepartureStation"].ToString());
        ...
        public DataSet GetFlightInformationforFFM(string FlightOrigin, string FlightNo, DateTime FlightDate)
        {
            DataSet ds = new DataSet();
            try
            {
        ...
        public DataSet GetRecordforGenerateFFM(string DepartureAirport, string FlightNo, DateTime FlightDate, string FlightDestination)
        {
            DataSet dsData = new DataSet();
            try
            {
        ...
        public bool EncodeFSAForSend(string AirlinePrefix, string AWBNo, string CarrierCode, string FlightNo, string OperType, string FromEmailID, string ToEmailID, string AWBPrefix)
        {
            bool flag = false;
            try
            {
                DataSet dsAWB = GetAWBDetailsForFSA(AWBNo, OperType, AWBPrefix);
                DataSet dsFlight = getFlightDetailsForFSA(AWBNo, CarrierCode.Trim() + FlightNo.Trim(), AWBPrefix);
                if (dsAWB != null)
                {
        ...
        public DataSet getFBLData(string POL, string FlightNo, string FlightDate)
        {
            //bool flag = false;
            DataSet dsData = new DataSet();
            try
        ...
        public DataSet getFBLDataORG(string POL, string FlightNo, string FlightDate)
        {
            //bool flag = false;
            DataSet dsData = new DataSet();
            try
        ...
        public DataSet getFFMUnloadingPort(string DepartureAirport, string FlightNo, string FlightDate)
        {
            DataSet ds = new DataSet();
            try
            {
        ...
        public DataSet getUpdatedFFMData(string DepartureAirport, string FlightNo, string FlightDate)
        {
            DataSet ds = new DataSet();
            DataSet dsData = new DataSet();
            try
        ...
        public bool addMsgToOutBox(string subject, string Msg, string FromEmailID, string ToEmailID)
        {
            bool flag = false;
            try
            {
        ...
        public bool addMsgToOutBox(string subject, string Msg, string FromEmailID, string ToEmailID, string UpdatedBy, DateTime? UpdatedOn, string Type = "", string AWBNumber = "", string FlightNumber = "", string FlightDate = "1900-01-01", string FlightOrigin = "", string FlightDestination = "")
        {
            bool flag = false;
        ...
        public bool InsertFFAInOutbox(string arr, string UserName)
        {
            bool flag = false;
            try
            {
        ...
        public DataSet getFFMData(string DepartureAirport, string FlightNo, string FlightDate)
        {
            DataSet dsData = new DataSet();
            try
            {
        ...
        public bool EncodeFHLForSend(string AWBNo, string FromEmailID, string ToEmailID)
        {
            bool flag = false;
            try
            {
        ...
        public DataSet GetFHLData(string MAWBNo)
        {
            try
            {
                DataSet ds = new DataSet();
        ...
        public bool EncodeFFRForSendOLD(DataSet ds, int refNO)
        {
            bool flag = false;
            try
            {
        ...
        public bool EncodeFHLForSend(DataSet ds, int refNo)//(string AWBNo, string FromEmailID, string ToEmailID)
        {
            bool flag = false;
            try
            {
        ...
        public bool EncodeFBLForSend(string POL, string FlightNo, string FlightDate, string FromEmailID, string ToEmailID, int refNo)
        {
            MessageData.fblinfo objFBLInfo = new MessageData.fblinfo("");
            MessageData.unloadingport[] objUnloadingPort = new MessageData.unloadingport[0];
            MessageData.consignmnetinfo[] objConsInfo = new MessageData.consignmnetinfo[0];
                    ...
                    if (dsData.Tables.Count > 1)
                    {
                        ...
                        if (count1 > 0 && count2 > 0)
                        {
                            ...
                            if (FBLMsg != null)
                            {
                                ...
                                {
                                    //bool flag = false;
                                    flag = addMsgToOutBox("FBL", FBLMsg, FromEmailID, ToEmailID, "", refNo);
                                }
                            }
                        }
                        else
        ...
        public bool EncodeFFMForSend(string DepartureAirport, string FlightNo, string FlightDate, string FromEmailID, string ToEmailID, int refNo)
        {
            bool flag = false;
            try
            {
                    ...
                    if (dsData.Tables.Count > 0)
                    {
                        ...
                        if (count1 > 0 && count2 > 0)
                        {
                            ...
                            if (FFMMsg != null)
                            {
                                ...
                                {
                                    //bool flag = false;
                                    flag = addMsgToOutBox("FFM", FFMMsg, FromEmailID, ToEmailID, "", refNo);
                                }
                            }
                        }
                        else
        ...
        public DataSet GetChildHAWB(string awbPrefix, string awbNumber, string flightNo, DateTime flightDate, bool manifestedHawb, string POL = "", bool IsXFZB = false)
        {
            try
            {
        ...
        private bool ValidateAndSaveCSNMessage(int refNO, ref MessageData.CSNInfo csnInfo, ref MessageData.customsextrainfo[] custominfo, string strMessageFrom, string strFromID, DataTable dtOCIInfo)
        {
            ...
            try
            {
                genericFunction.UpdateInboxFromMessageParameter(refNO, csnInfo.awbPrefix + "-" + csnInfo.awbNumber, csnInfo.flightNumber, csnInfo.pol, csnInfo.pou, "CSN" , strMessageFrom == "" ? strFromID : strMessageFrom, DateTime.Parse( flightDate));
