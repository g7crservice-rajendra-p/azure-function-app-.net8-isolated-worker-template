...
namespace QidWorkerRole
{
    public class DataBaseMessageProcessor
    {
        ...
        private void IncomingMessageProcesseandUpdatetheDatabase()
        {
            try
            {
                ...
                if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                {
                    for (int row = 0; row < ds.Tables[0].Rows.Count; row++)
                    {
                        ...
                        if (message.Contains("ZCZC") && message.Contains("NNNN"))
                            message = genericFunction.ExtractFromString(message, "ZCZC", "NNNN");

                        if (message.Contains("=SMI"))
                            message = genericFunction.ExtractFromString(message, "=SMI", "");
                        else if (message.Contains("=TEXT"))
                            message = genericFunction.ExtractFromString(message, "=TEXT", "");
                        message = genericFunction.ReplaceBlankSpaces(message);
        ...
        private void AutogenearateMessageandsend()
        {
            ...
            try
            {
                ...
                string f_AutoFBL;
                f_AutoFBL = genericFunction.ReadValueFromDb("AUTOFBL");
