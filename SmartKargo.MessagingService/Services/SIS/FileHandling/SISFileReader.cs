...
namespace QidWorkerRole.SIS.FileHandling
{
    public class SISFileReader
    {
        ...
        private readonly ISValidationReportReader _iSValidationReportReader;
        public SISFileReader(
            ...
            ISValidationReportReader iSValidationReportReader   
            )
        {
            _logger = logger;
            ...
            _iSValidationReportReader = iSValidationReportReader;
        }
        ...
        public async Task<(bool success, string logFilePath)> ReadSISFile(string filePath, string CreatedBy, string logFilePath)
        {
            ...
            try
            {
                ...
                //SIS.DAL.ReadDBData _readDBData = new DbEntity.ReadDBData();

                if (_readDBData.IsFileAlreadyExistsWithSameName(fileName))
                {
                ...
                else
                {
                    ...
                    if (fileExtention != null && fileExtention.ToUpper().Equals(".DAT"))
                    {
                        ...
                        foreach (var invoice in _idecFileReader.Read(filePath))
                        {
                            if (invoice != null)
                            {
                                ...
                                if (_readDBData.IsInvoiceAlreadyExists(invoice))
                                {
                    ...
                    if (fileData != null)
                    {
                        if (fileData.InvoiceList.Count > 0)
                        {
                            ...
                            if (_createDBData.InsertReceivedFileData(fileData, CreatedBy, out newFileHeaderId))
                            {
                                // Logger.InfoFormat("======================================================================================================");
                                ...
                                if (!string.IsNullOrWhiteSpace(logFileName) && !string.IsNullOrWhiteSpace(logFilePath))
                                {
                                    ...
                                    if (newFileHeaderId > 0)
                                    {
                                        ...
                                        _updateDBData.UpdateReceivedFileHeaderData(newFileHeaderId, DBNull.Value.ToString(), logFileBlobUrl);
                ...
                if (ValidationFilePathR1 != null)
                {
                    if (ValidationFilePathR1.Count() == 1)
                    {
                        if (!string.IsNullOrWhiteSpace(ValidationFilePathR1[0]))
                        {
                            ...
                            listISValidationSummaryReport = _iSValidationReportReader.ReadISValidationSummaryReportR1(ValidationFilePathR1[0]);
                            ...
                            if (listISValidationSummaryReport.Count > 0)
                            {
                                ...
                                if (ValidationFilePathR2 != null)
                                {
                                    if (ValidationFilePathR2.Count() == 1)
                                    {
                                        ...
                                        {
                                            listISValidationDetailErrorReport = _iSValidationReportReader.ReadISValidationDetailErrorReportR2(ValidationFilePathR2[0]);
                                        }
                                        else
                                        {
                                ...
                                if (_createDBData.UpdateStatusAndInsertISValidationReportR1R2(listISValidationSummaryReport, listISValidationDetailErrorReport, UserName, rejectionOnValidationFailure, onlineCorrectionAllowed, ref strAzulOracleInvList))
                                {
                                    ...
                                    if (!string.IsNullOrWhiteSpace(logValFileName) && !string.IsNullOrWhiteSpace(logValFilePath))
                                    {
                                        ...
                                        _createDBData.CreateReceivedISValidationFileHeaderData(Path.GetFileNameWithoutExtension(zipFileNameWithPath) + ".ZIP", DBNull.Value.ToString(), logValFileBlobUrl, receivablesFileID, UserName);
                                    }
